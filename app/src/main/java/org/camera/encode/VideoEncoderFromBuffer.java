...
import org.camera.pusher.StreamingPusher;
import org.camera.pusher.YUVPackage;
...
public class VideoEncoderFromBuffer {
	...
	public VideoEncoderFromBuffer(int width, int height) {
		...
		String rawfilename = DEBUG_FILE_NAME_BASE + this.mWidth + "x"
				+ this.mHeight + ".h264";

		if(true == mDumpRawData){
			try {
				mFileOutputStream = new FileOutputStream(rawfilename);
			} catch (IOException e) {
